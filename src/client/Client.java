...
public class Client {
	...
	private void run() {
		...
		try {
			...
			setSession(getCon().createTopicSession(false,
		...
		try {
			for (TopicSubscriber sub : ts) {
				...
				if (tm != null) {
					do {
						System.out
								.println(sub.getTopic().getTopicName() + " :");
	...
	private void miseAJourAbo() {
		try {
			...
			for (String s : me.getAbo()) {
				ts.add(getSession().createSubscriber(
						getSession().createTopic(s)));
			}
